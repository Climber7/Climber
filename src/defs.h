//
// Created by Climber on 2020/6/3.
//

#ifndef CLIMBER_EVENT_H
#define CLIMBER_EVENT_H

#include <wx/defs.h>

enum {
    ID_MENU_STATUS = wxID_HIGHEST + 1,
    ID_MENU_TOGGLE,
    ID_MENU_PROXY_DIRECT_MODE,
    ID_MENU_PROXY_PAC_MODE,
    ID_MENU_PROXY_GLOBAL_MODE,
    ID_MENU_SCAN_QR_CODE,
    ID_MENU_IMPORT_FROM_URL,
    ID_MENU_SHARE_CURRENT_SERVER,
    ID_MENU_SERVERS_SETTINGS,
    ID_MENU_PREFERENCES,
    ID_MENU_COPY_TERMINAL_PROXY_COMMAND_SHELL,
    ID_MENU_COPY_TERMINAL_PROXY_COMMAND_BASH,
    ID_MENU_COPY_TERMINAL_PROXY_COMMAND_CMD,
    ID_MENU_SHOW_LOG_CLIMBER,
    ID_MENU_SHOW_LOG_SS,
    ID_MENU_SHOW_LOG_SSR,
    ID_MENU_SHOW_LOG_VMESS,
    ID_MENU_SHOW_LOG_TROJAN,
    ID_MENU_CHECK_FOR_UPDATES,
    ID_MENU_ABOUT,
    ID_MENU_EXIT,

    ID_FRAME_PREFERENCES,
    ID_FRAME_ABOUT,
    ID_FRAME_SERVER_CONF,

    ID_PREFERENCES_MAIN_TAB,
    ID_CHOICE_LANGUAGE,
    ID_CHECK_BOX_AUTO_START,
    ID_CHECK_BOX_SHARE_ON_LAN,
    ID_SPIN_SOCKS_PORT,
    ID_SPIN_HTTP_PORT,
    ID_SPIN_PAC_PORT,
    ID_BUTTON_APPLY_PROXY_SETTINGS,
    ID_BUTTON_CANCEL_PROXY_SETTINGS,
    ID_DATA_VIEW_SERVER_LIST,

    ID_BUTTON_NEW_SERVER,
    ID_BUTTON_EDIT_SERVER,
    ID_BUTTON_DELETE_SERVER,
    ID_BUTTON_DUPLICATE_SERVER,
    ID_BUTTON_MOVE_UP_SERVER,
    ID_BUTTON_MOVE_DOWN_SERVER,


};

enum {
    DIRECT_PROXY_MODE,
    PAC_PROXY_MODE,
    GLOBAL_PROXY_MODE,
};


#define DEFAULT_PROXY_MODE (DIRECT_PROXY_MODE)

#define DEFAULT_LANGUAGE ("en_US")
#define DEFAULT_AUTO_START (false)
#define DEFAULT_SHARE_ON_LAN (false)
#define DEFAULT_SOCKS_PORT (1080)
#define DEFAULT_HTTP_PORT (8118)
#define DEFAULT_PAC_PORT (8119)


#endif //CLIMBER_EVENT_H
